...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_MattLeaman')
	 Begin 
	  Alter Database [Assignment06DB_MattLeaman] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_MattLeaman;
	 End
	Create Database Assignment06DB_MattLeaman;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_MattLeaman;
...
AS
  ...
  LEFT JOIN dbo.vEmployees
  ON dbo.vInventories.EmployeeID = dbo.vEmployees.EmployeeID
  GROUP BY dbo.vInventories.InventoryDate
  , dbo.vEmployees.EmployeeFirstName + ' ' + dbo.vEmployees.EmployeeLastName
  ORDER BY dbo.vInventories.InventoryDate
;
GO
...
FROM dbo.vCategories
  ...
  ON dbo.vInventories.EmployeeID = E.EmployeeID
  INNER JOIN dbo.vEmployees AS M
  ON E.ManagerID = M.EmployeeID
ORDER BY dbo.vCategories.CategoryID
, dbo.vProducts.ProductID
, dbo.vInventories.InventoryID
, dbo.vInventories.EmployeeID
;
